...
for alpha in alphas:
    ridge = Ridge(alpha=alpha, max_iter=10000, tol=1e-4) 
    ...
    iterations = ridge.n_iter_ if ridge.n_iter_ is not None else 'N/A'
...
print("\nТаблиця порівняння для різних значень alpha (Ridge):")
print(results_df)

best_alpha = results_df.loc[results_df['MSE'].idxmin(), 'Alpha']
ridge_best = Ridge(alpha=best_alpha, max_iter=10000, tol=1e-4)
ridge_best.fit(X_train, y_train)
feature_names = num_cols + list(encoder.get_feature_names_out(cat_cols))
coefficients = pd.Series(ridge_best.coef_, index=feature_names)
print(f"\nКоефіцієнти для найкращої моделі (alpha={best_alpha}):")
